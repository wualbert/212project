...
def main():
    ...
    C = np.matrix([[0, 1, 0, 0]])
    D = np.matrix([0])
    K = np.matrix([[5.0376, 0.02870, 5.0862, 0.03039]]) #swap the 2nd and 3rd K values from
    #K = np.matrix([[6.0376, 0.2870, 6.0862, 0.3039]]) #swap the 2nd and 3rd K values from
    ...
    global uPub
    uPub = rospy.Publisher('/uPub', Float32, queue_size = 1)

    # initalize encoder subscriber
    global encSub
    encSub = rospy.Subscriber('/encPub', Float32MultiArray, callback)

    # initialize reference subscriber
    global posSub
    posSub = rospy.Subscriber('/posPub', Float32, posCallback)
    
    # spin ros
    rospy.spin()

def posCallback(reference):
    global r #changes the reference position
    r = float(reference.data)
    print "set reference to " + str(r)
...
def callback(encData):
    ...
    # because we are sensing every state, our y vector is identical to our state vector x. thus we
    # can read these values into an x vector.
    global x
