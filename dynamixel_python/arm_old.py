...
class RobotArm:
    def __init__(self, simulated = False):
        ...
        self.gripperMin = -np.pi*5/6
        self.gripperMax = np.pi*5/6
    ...
    def setInitialParameters(self):
        ...
        for motorID in self.motors.keys():
            ...
            self.motors[motorID].max_torque = 800
        return True
    ...
    def _angleToMotor(self, robotPose):
        ...
        return (shoulder, int(elbowAngle/self.MXprecision)+2048, int(wristAngle/self.MXprecision)+2048)
    ...
    def _motorToAngle(self, motorPose):
        ...
        """
        return (0, (motorPose[1]-2048)*self.MXprecision, (motorPose[2]-2048)*self.MXprecision)
    ...
    def forwardKinematics(self, robotPose):
        ...
        """
        theta1, theta2, theta3 = robotPose
        return (self.l1*np.cos(theta1)+self.l2*np.cos(theta1+theta2)+self.l3*np.cos(theta1+theta2+theta3),
    ...
    def jacobian(self, robotPose):
        ...
        """
        theta1, theta2, theta3 = robotPose
        jacobianList = [-self.l1*np.sin(theta1)-self.l2*np.sin(theta1+theta2)-self.l3*np.sin(theta1+theta2+theta3),
    ...
    def setTargetPosition(self, targetPose, favorLeft = None):
        ...
        if self._robotPoseDistance(candidatePose[0], self.realThetaPose) < self._robotPoseDistance(candidatePose[1], self.realThetaPose):
            return self.setTargetAngle(candidatePose[0])
        else:
            return self.setTargetAngle(candidatePose[1])
